<template>
  <svg
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M14.5 11L20 11V18C20 20.2091 18.2091 22 16 22H8C5.79086 22 4 20.2091 4 18L4 11H9.5"
      stroke="#B6ADEA"
      stroke-width="1.5"
    />
    <path
      d="M16.5 11H19.5C20.3284 11 21 10.3284 21 9.5V7.5C21 6.67157 20.3284 6 19.5 6L4.5 6C3.67157 6 3 6.67157 3 7.5L3 9.5C3 10.3284 3.67157 11 4.5 11H7.5"
      stroke="#B6ADEA"
      stroke-width="1.5"
    />
    <path
      d="M12 4.5C12 3.11929 13.1193 2 14.5 2H15C16.1046 2 17 2.89543 17 4V4C17 5.10457 16.1046 6 15 6H12V4.5Z"
      stroke="#B6ADEA"
      stroke-width="1.5"
    />
    <path
      d="M12 4.5C12 3.11929 10.8807 2 9.5 2H9C7.89543 2 7 2.89543 7 4V4C7 5.10457 7.89543 6 9 6H12V4.5Z"
      stroke="#B6ADEA"
      stroke-width="1.5"
    />
    <path
      d="M14.5 6L14.5 22L9.5 22L9.5 6L14.5 6Z"
      stroke="#B6ADEA"
      stroke-width="1.5"
    />
  </svg>
</template>

<script>
export default {};
</script>

<style>
</style>